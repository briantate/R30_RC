  ...
  switch (appState) {
    ...
    case SEND: {
      ...
      bool res = MiApp_SendData(SHORT_ADDR_LEN, (uint8_t*)&broadcastAddress,
                                SEND_BUFFER_SIZE, sendDataBuffer,
                                msghandledemo++, true, false, dataConfcb);
      if (!res) {
        printf("send fail\r\n");
      }
      ...
      while (txComplete != true) {
        ...
        if (timeoutCount > 0xFF)  // ToDo: sometimes dataConfCallback is not
...
// 	system_gclk_chan_set_config(OSCCTRL_GCLK_ID_DFLL48,
// &dfll_gclk_chan_conf); 	system_gclk_chan_enable(OSCCTRL_GCLK_ID_DFLL48);
